...
import { NODE_REGISTRY, type NodeType } from '../../types/workflow';
  ...
  return (
    <aside className="w-64 bg-white border-r border-gray-200 p-4 flex flex-col h-full">
      ...
      <div className="flex flex-col gap-3">
        {(Object.keys(NODE_REGISTRY) as NodeType[]).map(key => (
          <div 
            key={key}
            ...
            draggable
          >
            <span className="text-sm font-medium text-gray-700">{NODE_REGISTRY[key].label}</span>
          </div>
        ))}
      </div>
    </aside>
